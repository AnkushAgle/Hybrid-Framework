...
public class Uploadimg extends BaseClass {
	...
	public void uploadfile() throws Exception {
		...
		String testcasename="Upload file into item master basic entry";
		
		//1)Login into application	
			
			Library.login("material","uploadfile",testcasename);
	    ConfigDataProvider config=new ConfigDataProvider("material","uploadfile");
	
	
	Library.filelocationConfigreport("material","uploadfile");
	...
	WebElement file=ele.getDrawingfile();
	...
	JavascriptExecutor js = (JavascriptExecutor)driver;
		...
		Library.custom_Sendkeys( ele.getItemDetailDescription(), config.getconfigdata("ItemDetailDesc"), "ItemDetailDesc");
		 Library.custom_Sendkeys( ele.getItemaddDetailDesc(), config.getconfigdata("ItemaddDetailDesc"),"ItemaddDetailDesc" );
			...
			String imgpath="D:\\ANKUSH\\NEW_HORIZON\\ExcelFiles\\NH.xlsx";
			ele.getUnitOfMeasurement().sendKeys("D:\\ANKUSH\\\\NEW_HORIZON\\\\ExcelFiles\\\\NH.xlsx");
...
//		  //act.moveToElement(upload).click().perform();
		...
		ele.getFileupload().click();
		 ...
		 UploadFile.roboupload(config.getconfigdata("filename"));
...
//			rb.keyRelease(KeyEvent.VK_ENTER);
			...
			Library.Comboclick(Filetype,config.getconfigdata("NatureofFile"), "Filetype");
